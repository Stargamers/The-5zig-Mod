...
public class ClassProxy {
	...
	static {
		...
		try {
			...
			if (Transformer.FORGE) {
				...
				worldData = cnd.class.getDeclaredField("field_186786_g");
				...
				resourcePackList = cgv.class.getDeclaredField("v");
				worldData = cnd.class.getDeclaredField("i");
	...
	public static void patchMainMenu(Object instance, int paramInt1, int paramInt2, boolean isForge) {
		...
		guiMainMenu.a((cgu) button);
		List<cgu> list;
		if (Transformer.FORGE) {
			try {
				list = ((List<cgu>) buttonList.get(guiMainMenu));
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
		} else {
	...
	public static IButton getMCPVPButton(ckd guiScreen) {
	...
	public static void guiMultiplayerActionPerformed(ckd guiScreen, cgu button) {
		if (button.k == 9) {
//			The5zigMod.getVars().displayScreen(new ServerListMCPvP(guiScreen));
	...
	public static void fixOptionButtons(Object instance) {
		...
		if (Transformer.FORGE) {
			try {
				list = ((List<cgu>) buttonList.get(guiScreen));
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
		} else {
	...
	public static void handleGuiResourcePackInit(Object instance, Object listObject, Object listObject2) {
		...
		List<cnb> list2;
		try {
			list = (List<cnb>) ClassProxy.resourcePackList.get(listObject);
			list2 = (List<cnb>) ClassProxy.resourcePackList.get(listObject2);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		Comparator<cnb> comparator = new Comparator<cnb>() {
			@Override
			public int compare(cnb o1, cnb o2) {
		...
		MinecraftFactory.getClassProxyCallback().addSearch(
				new SearchEntry<cnb>(MinecraftFactory.getVars().createTextfield(MinecraftFactory.getClassProxyCallback().translate("gui.search"), 9991, gui.m / 2 - 200, gui.n - 70, 170, 16),
						list, comparator) {
					@Override
					public boolean filter(String text, cnb o) {
				...
				new SearchEntry<cnb>(MinecraftFactory.getVars().createTextfield(MinecraftFactory.getClassProxyCallback().translate("gui.search"), 9992, gui.m / 2 + 8, gui.n - 70, 170, 16),
						list2, new Comparator<cnb>() {
					@Override
					public int compare(cnb o1, cnb o2) {
						return 0;
					}
	...
	public static void handleGuiSelectWorldInit(Object instance, List l) {
		final cnb guiSelectWorld = (cnb) instance;
